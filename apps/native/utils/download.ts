  ...
  return MediaLibrary.requestPermissionsAsync()
    .then((permission) => {
      if (!permission.granted) {
        return Promise.reject(new Error("Storage permission is required to save stickers to your gallery"));
      }
    ...
    .then((bundle) => {
      if (!bundle || !bundle.stickers || bundle.stickers.length === 0) {
        return Promise.reject(new Error("Pack has no stickers or failed to fetch bundle"));
      }
      ...
      const downloadAndSavePromises = bundle.stickers.map((sticker, i) => {
        if (!sticker.url) {
          return Promise.reject(new Error(`Sticker ${i} has no valid remote URL`));
        }
        ...
        return FileSystem.downloadAsync(sticker.url, tempCacheUri)
          .then((downloadResult) => {
    ...
    .then(() => console.log("Sticker pack successfully exported into device gallery app"));
}