...
export function CreatePackForm({ onSuccess, tags, onTagsChange }: CreatePackFormProps) {
	...
	const createPackMutation = useMutation(
		orpc.packs.create.mutationOptions({
			onSuccess: () => {
			...
			onError: (err: any) => {
				setErrorMsg(err.message || "Failed to create pack. Check your rate limits!");
	...
	const pickImages = async () => {
		...
		if (status !== "granted") {
			Alert.alert("PERMISSION DENIED", "We need library permissions to pick stickers!");
			return;
		}
	...
	const handleCreatePack = async () => {
		...
		setErrorMsg(null);
		createPackMutation.mutate(undefined);

		try {
			// Use native FormData to bypass ORPC's blob-creating serializer.
			// Hermes doesn't support Blob constructor from ArrayBuffer,
			// but response.blob() works fine.
			...
			// Use native fetch instead of ORPC to avoid blob serialization issues
			const apiUrl = process.env.EXPO_PUBLIC_API_URL || "http://localhost:3000";
			const response = await fetch(`${apiUrl}/api/packs/create-formdata`, {
				method: "POST",
				headers: {
					Authorization: `Bearer ${await (await import("@react-native-async-storage/async-storage")).default.getItem("auth_token")}`,
				},
				body: formData,
			});
			...
			const errorMsg = e instanceof Error ? e.message : "Failed to prepare images for upload!";
		...
		<View style={styles.container}>
			...
			<ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
				...
				<View style={styles.inputGroup}>
					...
					<TouchableOpacity style={styles.tagsBtn} onPress={() => setTagsSheetVisible(true)}>
				...
				<View style={styles.inputGroup}>
					<View style={styles.stickerHeaderRow}>
						<Text style={styles.label}>STICKERS ({selectedImages.length}/30) *</Text>
					...
					{selectedImages.length > 0 ? (
						<View style={styles.selectedGrid}>
							...
							{selectedImages.length < 30 && (
								<TouchableOpacity style={styles.addStickerGridBtn} onPress={pickImages}>
						...
						<TouchableOpacity style={styles.pickerArea} onPress={pickImages}>
							...
							<Text style={styles.pickerSub}>WEBP, PNG, GIF · UP TO 30 STICKERS · 2MB EACH</Text>
						</TouchableOpacity>
					)}
				</View>
				...
				{/* Submit button */}
				<TouchableOpacity
					style={[styles.submitButton, createPackMutation.isPending && styles.disabledButton]}
